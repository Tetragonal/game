...
public class MapOne implements Screen {
	...
	public void show() { //create()
		...
		player = new Player(layer, 19*layer.getTileWidth(), (layer.getHeight()-14)*layer.getTileHeight());
...
//			camera.position.set(CAMERA_OFFSET_X/2*camera.zoom + player.getX() + player.getWidth()/2, player.getY() + player.getHeight()/2, 0);
		...
		if(player.warpDestination == "MapTwo")
		{
			player.disposeAssets();
			game.setScreen(new MapTwo());
		}
	}
